...
// Define a type for the settings response
...
// Define a type for updating settings
 ...
 * Get user settings
 */
export const GET = withAuth<SettingsResponse>(
 ...
 */
export const PUT = withAuth<SettingsResponse>(
  async (req: NextRequest, userId: string) => {
    try {
      await dbConnect();
      ...
      if (settings.lengthUnit !== undefined) {
        if (typeof settings.lengthUnit !== 'string' || !['cm', 'in'].includes(settings.lengthUnit)) {
          return apiError('Length unit must be either "cm" or "in"', 400, 'ERR_VALIDATION');
        }
      }
      
      if (settings.theme !== undefined && typeof settings.theme !== 'string') {
        return apiError('Theme must be a string', 400, 'ERR_VALIDATION');
      }
