// src/components/domains/trophe/FoodEditor.tsx
'use client'
...
export function FoodEditor({ food, onSave, onCancel }: FoodEditorProps) {
  const { updateFood } = useNutritionStore()
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await updateFood(food.id, updates)
      if (result) {
        onSave()
      }
    } catch (error) {
    ...
    <Card className="p-6 border-2 border-orange-200 bg-orange-50/50">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-medium">Edit Food: {food.name}</h3>
