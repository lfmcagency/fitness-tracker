...
// For chart rendering - in a real implementation, you'd use a charting library
// like Recharts, Chart.js, or D3.js. This is a simplified representation.
...
export function MetricChart({
  ...
  allowExpand = true,
  height = 200,
  colorPalette = ['#1A1A1A', '#7D8F69', '#A4907C', '#B85C38', '#6B6B6B'],
  className,
  loading = false,
  showEmptyState = true,
  emptyStateContent,
  ...
  return (
    <div className={cn(
      'border border-[#E5E0DC] rounded-lg overflow-hidden bg-white',
      isExpanded && 'fixed inset-4 z-50 flex flex-col',
      className
    )}>
      {/* Chart header */}
      <div className="border-b border-[#E5E0DC] p-3 bg-[#F7F3F0]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <ChartIcon className="w-4 h-4 text-[#6B6B6B]" />
          ...
          <div className="flex items-center gap-1">
            ...
            {allowChartTypeChange && (
              <div className="flex items-center border border-[#E5E0DC] rounded-md overflow-hidden">
                <button
                  type="button"
                  className={cn(
                    "p-1",
                    currentChartType === 'line' ? 'bg-[#E5E0DC]' : 'bg-white'
                  )}
                  onClick={() => handleChartTypeChange('line')}
                >
                  <LineChartIcon className="w-3.5 h-3.5 text-[#1A1A1A]" />
                </button>
                <button
                  type="button"
                  className={cn(
                    "p-1",
                    currentChartType === 'bar' ? 'bg-[#E5E0DC]' : 'bg-white'
                  )}
                  onClick={() => handleChartTypeChange('bar')}
                >
                  <BarChartIcon className="w-3.5 h-3.5 text-[#1A1A1A]" />
                </button>
                <button
                  type="button"
                  className={cn(
                    "p-1",
                    currentChartType === 'pie' ? 'bg-[#E5E0DC]' : 'bg-white'
                  )}
                  onClick={() => handleChartTypeChange('pie')}
                >
                  <PieChartIcon className="w-3.5 h-3.5 text-[#1A1A1A]" />
                </button>
              </div>
            )}
            ...
            {allowExpand && (
              <button
                type="button"
                className="p-1 hover:bg-[#E5E0DC] rounded-md"
                onClick={toggleExpand}
              >
                {isExpanded 
                  ? <Minimize className="w-4 h-4 text-[#1A1A1A]" /> 
                  : <Maximize className="w-4 h-4 text-[#1A1A1A]" />
                }
              </button>
            )}
          </div>
      ...
      >
        {loading ? (
          <div className="absolute inset-0 flex items-center justify-center bg-white/75">
            <div className="w-8 h-8 border-4 border-[#E5E0DC] border-t-[#1A1A1A] rounded-full animate-spin"></div>
          </div>
          ...
          <div className="w-full h-full">
            ...
            {(xAxisLabel || yAxisLabel) && (
              <div className="text-xs text-[#6B6B6B] flex justify-between mt-2">
...
const SimplifiedLineChart: React.FC<{ 
  ...
  return (
    <div className="w-full h-full relative">
      {/* Y-axis */}
      <div className="absolute left-0 top-0 bottom-0 w-6 border-r border-[#E5E0DC] flex flex-col justify-between text-xs text-[#6B6B6B]">
        <div>{maxValue}</div>
        <div>0</div>
      </div>
      ...
      {/* X-axis labels */}
      <div className="absolute left-6 right-0 bottom-0 h-6 flex justify-between text-xs text-[#6B6B6B]">
...
const SimplifiedBarChart: React.FC<{ 
  ...
  return (
    <div className="w-full h-full relative">
      ...
      {/* X-axis labels */}
      <div className="absolute left-6 right-0 bottom-0 h-6 flex justify-around text-xs text-[#6B6B6B]">
        {data.slice(0, 10).map((point, index) => (
...
const SimplifiedPieChart: React.FC<{ 
  ...
  return (
    <div className="w-full h-full flex items-center justify-center">
      ...
      {/* Legend */}
      <div className="text-xs text-[#6B6B6B] ml-4">
        {slices.map((slice, index) => (
